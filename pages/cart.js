    ...
    export default function CartPage() {
        ...
        if(goToOrder){
          ...
          const handleContinue = async () => {
            const saveInfoData = {
              packageId: "discover-global-15days-3gb-topup",
              isdn: "95341613",
              countryCode: "MN",
              type: "GESIM",
              price: total,
              lang: "mn",
              vendor: "AIRALO",
              email: 'emonh7066@gmail.com',
            };
            try {
                const response = await saveInfo(saveInfoData);
              console.log("saveinfo" + JSON.stringify(response));
              if (
                ...
                const payload = {
                  isdn: Buffer.from(isdn ?? "", "base64").toString("ascii") ?? "",
                  products: [
                    {
                      productType: "GESIM",
                      price: total,
                      quantity: 1,
                      productCode: response.data.id,
                    },
                  ],
                  customerType:
                    "CONSUMER",
                  languageCode: 'mn',
                  vatEmail: 'emonh7066@gmail.com',
                  corpId: "",
                  device: "WEB",
                  redirectUrl: pkg.id ?? "",
                };
                    ...
                    if (isWebView) {
                      const data = {
                        ...
                        value: {
                          ...
                          register: "",
                          vatType: "CONS",
                          activationCode: res?.invoiceId,
                          productName: 'MkL commerce product',
                        },
                      };
                      console.log(data);
                      let webView;
                      if (window.mobiBridge !== undefined) {
                        webView = window.mobiBridge;
                      }
